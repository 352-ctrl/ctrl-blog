package com.example.blog.dto.job;
...
public class SysJobUpdateDTO {
    ...
    @Schema(description = "任务ID", example = "1623456789012345678", requiredMode = Schema.RequiredMode.REQUIRED)
    @NotNull(message = "任务ID不能为空")
    @Positive(message = "任务ID必须为正整数")
    private Long id;
    ...
    @Schema(description = "计划执行错误策略 (1:立即执行 2:执行一次 3:放弃执行)", example = "3")
    ...
    @Schema(description = "备注信息", example = "每10分钟同步一次文章阅读量到数据库")
