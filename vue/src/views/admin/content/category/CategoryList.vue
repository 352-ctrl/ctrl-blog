<template>
  <div>
    ...
    <el-dialog v-model="data.formVisible" :title="data.form.id ? '编辑分类' : '新增分类'" class="dialog-md-down" width="500px">
...
const submitForm = () => {
  formRef.value.validate((valid) => {
    if (valid) {
      const apiMethod = data.form.id ? updateCategory : addCategory;
...
// 批量删除
const batchDeleteCategoryApi = async (selectedIds) => {
  return deleteCategories(selectedIds);
};
