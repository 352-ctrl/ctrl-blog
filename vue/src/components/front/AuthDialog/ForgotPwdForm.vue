...
import { sendForgotPwdEmailCode, resetPasswordByEmail } from "@/api/auth.js";
...
const onVerifySuccess = (params) => {
  ...
  sendForgotPwdEmailCode(requestData).then(res => {
    if (res.code === 200) {
  ...
  formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      try {
        ...
        const res = await resetPasswordByEmail(payload);
        if (res.code === 200) {
        ...
        ElMessage.error(error.msg || '系统异常，请联系管理员');
      } finally {
        loading.value = false;
      }
    }
